...
const salt = bcrypt.genSaltSync(10); // thuat toán hash password
let createNewUser = async (data) => {
  //hàm tạo user với tham số data
  return new Promise(async (resolve, reject) => {
    //dùng Promise đảm bảo luon trả kết quả, trong xử lý bat đồng bộ
    try {
...
//lấy tat ca findAll CRUD
let getAllUser = () => {
  return new Promise(async (resolve, reject) => {
    //dung Promise đảm bảo luôn trả kết quả, trong xử lý bat đồng bộ
    try {
...
let updateUser = (data) => {
  return new Promise(async (resolve, reject) => {
    //dung Promise đảm bảo luon tra ket quả, trong xử lý bat đồng bộ
    try {
      let user = await db.User.findOne({
...
let deleteUserById = (userId) => {
  return new Promise(async (resolve, reject) => {
    //dùng Promise đảm bảo luôn trả kết quả, trong xử lý bat đồng bộ
    try {
      let user = await db.User.findOne({
        where: { id: userId },
      });
